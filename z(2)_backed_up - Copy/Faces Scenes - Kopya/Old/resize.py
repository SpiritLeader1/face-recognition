...
target_width = 1792  # Change this to your desired width
target_height = 1024
# Load the image
root = os.getcwd()
...
for path, subdirs, files in os.walk(root):
    for name in files:
        if(bool(re.search("(png|jpeg|jpg)$",name)) and not bool(re.search(r"0_Face",path))):
...
# Save or display the resized image
            ...
            if((bool(re.search("(png|jpeg|jpg)$",name)) and not bool(re.search("new",name)))):
